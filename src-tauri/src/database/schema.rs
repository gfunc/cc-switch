...
impl Database {
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    11 => {
                        log::info!("迁移数据库从 v11 到 v12（添加项目 Profiles 表 + provider_endpoints 增加 last_used 字段）");
    ...
    /// v11 -> v12 迁移：添加项目 Profiles 表 + provider_endpoints 增加 last_used 字段
    ...
    fn migrate_v11_to_v12(conn: &Connection) -> Result<(), AppError> {
        ...
        log::info!("v11 -> v12 迁移完成：profiles 表 + provider_endpoints.last_used");
        Ok(())
    }
